...
var mods: Dictionary[int, ModTracker]
## Based on turn(), mods that are currently being applied.
var curr_mods:Array[Modifier]
## start id number
var start_id: int
...
func _init(prop_name:StringName, mods:Dictionary[int, ModTracker]={}, start_id:int=0) -> void:
	self.prop_name = prop_name
	self.mods = mods
	curr_mods = []
	start_id = 0
	curr_id = start_id
	self.connect("end_mod", _end_mod)
...
func _end_mod(id:int, curr_turn:int):
